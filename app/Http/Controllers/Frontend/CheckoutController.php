...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // $cart = $request->cart;
            $cart = Cart::with(['product', 'variant'])->whereIn('id', $request->cart)->get();
        }else if($request->isMethod('get')){
            if(Session::has('cart') && count(Session::get('cart')) > 0){
            //    $cart = ;
               $cart = Cart::with(['product', 'variant'])->whereIn('id', Session::get('cart'))->get();
            }else{
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            foreach($request->lines as $i){
                ...
                $line->unit_id = $i['variant']['sell_unit_id'];
                $line->price = $i['price'];
                $line->qty = $i['qty'];
                $line->subtotal = $i['price'] * $i['qty'];
